...
function getCoord(e) {
	var 	carPos = e.target.selectionEnd,
		div = document.createElement('div'),
		span = document.createElement('span'),
		copyStyle = getComputedStyle(e.target),
	...
	div.textContent = e.target.value.toString().substr(0, carPos);
	span.textContent = e.target.value.toString().substr(carPos) || '.';
	div.appendChild(span);

	emailCoords = getPosition(e.target);							//console.log("emailCoords.x: " + emailCoords.x + ", emailCoords.y: " + emailCoords.y);
...
// restrict cash with type of text just accept number
